...
function tester(testerNum, tileSize){
	...
	/*PATHFINDING VALUES:
		â€¢ 0 = explore random path until dead end, and restart.
		â€¢ 1 = explore every possibility of every node via stack
		â€¢ 2 = astar
	*/
	this.targetI = this.board.maze.length-1;//Math.floor(Math.random()*this.board.maze.length-1);
	this.targetJ = this.board.maze[0].length-1;//Math.floor(Math.random()*this.board.maze[0].length-1);
...
function mouse(board, startTile, targetTile, pathFindingType){
	...
	/*PATHFINDING VALUES:
		â€¢ 0 = queue approach.
		â€¢ 1 = stack approach, depth-first search.
		â€¢ 2 = A*
